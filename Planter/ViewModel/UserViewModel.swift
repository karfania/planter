...
//    }()
    ...
    private lazy var locationManager: CLLocationManager = {
        ...
        manager.distanceFilter = 1000 // update every 1km user moves
...
//    }
    ...
    func createUser(name: String, email: String, password: String) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
            ...
            // adding user to Firestore "users" collections
    ...
    func loginUser(email: String, password: String) {
        Auth.auth().signIn(withEmail: email, password: password) { authResult, error in
            ...
            self.getUserData(uid: fbUser.uid) { fetchedUser in
    ...
    func logoutUser() {
        do {
            // setting user to nil upon success
        ...
        Task {
            do {
                ...
                if let newPlantDetails = await plantViewModel.fetchPlantDetails(id: newPlant.id) {
                    let plant = Plant(id: newPlant.id, name: newPlant.name, cycle: newPlant.cycle, watering: newPlant.watering, description: newPlantDetails.description, attracts: newPlantDetails.attracts, default_image: newPlant.default_image, location_obtained: location)
                    plants.append(plant)
                    self.updateUser()
                    return
                }
                // plant has no details, leave blank
                let plant = Plant(id: newPlant.id, name: newPlant.name, cycle: newPlant.cycle, watering: newPlant.watering, description: "", attracts: [], default_image: newPlant.default_image, location_obtained: location)
                plants.append(plant)
                self.updateUser()
            }
        }
